        ...
        <Slider {...sliderSettings}>
            {heroGallery.items.map((item: any, index) => (
                <div key={item.id} className={styles.slider__item}>
                    ...
                    {index === 0 ?
                        <Image priority className={styles.image} src={`${heroGallery.path}${item.name}`} width={1920} height={1080}  alt={item.name}/>
                    :
                        <Image className={styles.image} src={`${heroGallery.path}${item.name}`} width={1920} height={1080}  alt={item.name}/>
                    }
                </div>
            ))}
        </Slider>
